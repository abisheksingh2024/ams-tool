...
export default function StepAsset({ form, updateForm, onNext, onBack }: Props) {
  const fullSymbol = `${form.symbolPrefix}${form.symbolSuffix}`.toLowerCase().replace(/\s+/g, '') || 'token';
      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-5">
          <FormField>
            ...
            <Input
              value={form.symbolPrefix}
              onChange={(v) => updateForm({ symbolPrefix: v })}
              placeholder="eth:"
              monospace
            />
            <HelpText>Chain-specific prefix for internal symbol resolution</HelpText>
          </FormField>
          <FormField>
        ...
        <FormField>
          ...
          <div className="flex items-center gap-2">
            <div className="flex-1 px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg font-mono text-sm text-gray-700">
              {fullSymbol}
            </div>
